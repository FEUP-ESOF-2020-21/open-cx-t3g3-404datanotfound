...
class CommentsPageState extends State<CommentsPage>{
 ...
 //add new comment to the database, refreshing the comments page
  ...
  Widget _buildCommentList(){
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index){
        ...
        String name = map.values.toList()[2][commentsAuthor[index]]["name"].toString();
        String userPhoto = map.values.toList()[2][commentsAuthor[index]]["photo"].toString();

        return _buildCommentItem(commentsText[index], name, userPhoto, commentID);
        }
    );
  }

  //create comment widget
  Widget _buildCommentItem(String comment, String name, String userPhoto, String commentID){
    return Card(child:
      ListTile(
  ...
  Future insertDatabase(String val) async {
    ...
    FirebaseDatabase.instance.reference().child('Posts').child(confId).child(postID).child("comments");
